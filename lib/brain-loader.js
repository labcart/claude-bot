...
class BrainLoader {
   ...
   *
   * @param {string} agentId - Agent UUID from database
   * @returns {Promise<Object|null>} Brain configuration or null if not found
   */
  async loadFromSupabase(agentId) {
    if (!supabase) {
      return null;
    }
    ...
    const { data, error } = await supabase
      ...
      .select('name, short_description, brain_config')
      .eq('id', agentId)
      .single();
    ...
    // brain_config contains: { systemPrompt, temperature, maxTokens, security, tts, etc. }
    ...
    return {
      ...
      version: '1.0',
      security: brainConfig.security || 'default',
      private: false,
