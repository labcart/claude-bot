...
async function sendSimple(options) {
  const { message, sessionId, claudeCmd, onStream, messageContent, onToolResult } = options;

  return new Promise((resolve, reject) => {
    // POC: Write MCP Router config to temp file (professional approach)
    const mcpConfigPath = path.join('/tmp', `mcp-router-${Date.now()}.json`);
    const mcpConfig = {
      mcpServers: {
        router: {
          command: 'node',
          args: ['/Users/macbook/play/mcp-router/index.js']
        }
      }
    };
    ...
    try {
      ...
      return reject({ success: false, error: `Failed to write MCP config: ${err.message}` });
    }
    ...
    const child = spawn(claudeCmd, args, {
      // Don't set cwd - use current working directory
      // Claude session isolation happens via session UUIDs, not directories
...
async function sendWithPermissions(options) {
  const { message, sessionId, claudeCmd, onStream, onPermissionRequest } = options;
...
async function sendToClaudeWithTTS(options) {
  ...
  return new Promise((resolve, reject) => {
    // POC: Write MCP Router config to temp file
    const mcpConfigPath = path.join('/tmp', `mcp-router-${Date.now()}.json`);
    const mcpConfig = {
      mcpServers: {
        router: {
...
async function sendToClaudeWithImage(options) {
  ...
  return new Promise((resolve, reject) => {
    ...
    let cmd = claudeCmd;
    if (sessionId) {
      // Turn 2 - resuming session, needs --ide for MCP tools
      ...
      console.log(`🚀 Spawning (Image mode Turn 2): ${claudeCmd} --ide --resume ${sessionId} --strict-mcp-config`);
    } else {
      // Turn 1 - new session, NO --ide flag to block MCP tool access
      ...
      console.log(`🚀 Spawning (Image mode Turn 1 - no tools): ${claudeCmd} --strict-mcp-config`);
    }
    ...
    child.stdout.on('data', (chunk) => {
      ...
      lines.forEach(line => {
        try {
          ...
          switch (data.type) {
            case 'system':
              if (data.message?.includes('initialized')) {
                console.log(`📋 Session initialized: ${data.model || 'unknown'}`);
                if (data.metadata?.sessionInfo?.sessionId) {
                  sessionInfo = data.metadata.sessionInfo;
                }
              }
              break;
            ...
            case 'result':
              if (turn === 1) {
                // Turn 1 complete - now request image generation
                console.log(`✅ Turn 1 complete (${data.duration_ms}ms) - text response received`);
                console.log(`🖼️  Requesting image generation...`);
                ...
                // Include style context if provided (from image profile or inline config)
                const styleContextSection = imagePromptContext
                  ? `\n\nSTYLE CONTEXT:\n${imagePromptContext}\n\nYou MUST follow the style context above when creating the prompt.\n`
                  : '';

                // Build the actual image prompt from user's request + style context
                const userRequest = userText || 'Generate an image';
                console.log(`🐛 DEBUG userText: ${userText?.substring(0, 100)}`);
                console.log(`🐛 DEBUG userRequest: ${userRequest.substring(0, 100)}`);
                const imagePrompt = imagePromptContext
                  ? `${userRequest}. Style: ${imagePromptContext}`
                  : userRequest;
                console.log(`🐛 DEBUG imagePrompt: ${imagePrompt.substring(0, 200)}`);
...
Call the tool now with these exact parameters. No additional response needed.`;
                ...
                // DEBUG: Log the exact Turn 2 message being sent
                console.log('📤 Turn 2 message being sent to Claude:');
                console.log(imageRequestText.substring(0, 500) + '...');
                console.log(`📎 Filename parameter: ${imageFilename}`);

                child.stdin.write(turn2Message + '\n');
                // Note: Don't close stdin yet - wait for Turn 2 result
